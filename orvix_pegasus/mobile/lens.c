 ...
 *   lens stream     URL [interval_ms]   capturas periódicas
 *   lens tap-stream URL [interval_ms]   periódicas + captura al tocar la pantalla
 *
...
static void process_event_line(const char *line, tap_state_t *st, const url_t *u) {
    if (strstr(line, "ABS_MT_POSITION_X")) {
        ...
        if (strstr(line, "DOWN")) st->touch_down = 1;
        else if (strstr(line, "UP")) st->touch_down = 0;
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr,
            ...
            "  %s stream     URL [interval_ms]   capture periodicas\n"
            "  %s tap-stream URL [interval_ms]   periodicas + por tap\n"
